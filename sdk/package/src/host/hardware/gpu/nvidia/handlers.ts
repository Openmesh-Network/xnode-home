...
import type { Gpu, GpuOptions, Info, Usage } from "./models.js";
import { scope, type Path } from "./scope.js";

export type nvidia_input = WithClient<{ query: GpuOptions }>;
export type nvidia_output = ResponseResult<rust_types.Vec<Gpu>>;
export async function nvidia(input: nvidia_input): Promise<nvidia_output> {
  return JsonGet(input, `${parentScope()}/nvidia`);
}
