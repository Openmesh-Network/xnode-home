...
export function scope<Path extends ContainerPath>(path: Path) {
  return parentScope(path) + "/process";
}

export type logs_input = WithClient<{
...
export async function logs(input: logs_input): Promise<logs_output> {
  return JsonGet(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/logs`
  );
}

export type status_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type status_output = ResponseResult<Status>;
export async function status(input: status_input): Promise<status_output> {
  return JsonGet(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/status`
  );
}

export type usage_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type usage_output = ResponseResult<Usage>;
export async function usage(input: usage_input): Promise<usage_output> {
  return JsonGet(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/usage`
  );
}

export type start_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type start_output = ResponseResult<Bytes>;
export async function start(input: start_input): Promise<start_output> {
  return RawPost(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/start`
  );
}

export type stop_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type stop_output = ResponseResult<Bytes>;
export async function stop(input: stop_input): Promise<stop_output> {
  return RawPost(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/stop`
  );
}

export type restart_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type restart_output = ResponseResult<Bytes>;
export async function restart(input: restart_input): Promise<restart_output> {
  return RawPost(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/restart`
  );
}

export type reload_input = WithClient<{ path: ContainerPath & ProcessPath }>;
export type reload_output = ResponseResult<Bytes>;
export async function reload(input: reload_input): Promise<reload_output> {
  return RawPost(
    input,
    (path) => `${scope(path)}/${encodeURIComponent(path.process)}/reload`
  );
}
